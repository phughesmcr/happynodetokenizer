...
/**
	 ...
	 */function qt(c){c=c.replace("&amp;"," and ");const r=/&#\d+;/.exec(c);r&&r.length>0&&r.forEach((r=>{c=c.replace(r,String.fromCharCode(parseInt(r.substring(2),10)))}));const e=/&\w+;/.exec(c);return e&&e.length>0&&e.forEach((r=>{c=c.replace(r,(c=>void 0!==rt[c]?String.fromCodePoint(...rt[c].codepoints):c)(r))})),c}function Et(c,r){return"dlatk"===r?pt.test(c)?"phone":ht.test(c)?"url":lt.test(c)?"url_scheme":ut.test(c)?"url_authority":ft.test(c)?"url_path_query":gt.test(c)&&(!dt.test(c)||dt.test(c)&&/[a-zA-Z]{2,}/g.test(c))?"htmltag":dt.test(c)?"emoticon":mt.test(c)?"username":bt.test(c)?"hashtag":vt.test(c)?/\w/.test(c)?"word":"punct":"<UNK>":et.test(c)?"phone":ot.test(c)&&(!tt.test(c)||tt.test(c)&&/[a-zA-Z]{2,}/g.test(c))?"htmltag":tt.test(c)?"emoticon":at.test(c)?"username":st.test(c)?"hashtag":nt.test(c)?/\w/.test(c)?"word":"punct":"<UNK>"}c.dlatkTokenizerPattern=wt,c.stanfordTokenizerPattern=it,c.tokenize=function(c,r,e){if(!c||"string"!=typeof c){if(e&&"function"==typeof e)return e("HappyNodeTokenizer: input must be a string.",[]);throw new TypeError("HappyNodeTokenizer: input must be a string.")}const t=function(c){const r={...yt,...c},e="string"!=typeof r.locale?"en-US":r.locale;r.locale=e;const t="string"!=typeof r.mode?"stanford":r.mode.toLowerCase();if(!/dlatk|stanford/.exec(t))throw new SyntaxError('Tokenizer mode must be "stanford" or "dlatk".');r.mode=t;const o="string"!=typeof r.normalize?void 0:r.normalize.toUpperCase();if(o&&!/NFK?(C|D)/.exec(o))throw new SyntaxError(`Tokenizer "normalize" option must be undefined | 'NFC' | 'NFD' | 'NFKC' | 'NFKD', found "${r.normalize??""}".`);return r.normalize=o,r.preserveCase=void 0===r.preserveCase||"boolean"!=typeof r.preserveCase||r.preserveCase,r.tag=void 0!==r.tag&&"boolean"==typeof r.tag&&r.tag,r}(r);let o,a=qt(c);if("dlatk"===t.mode&&(a=a.replace(/\\x[0-9a-z]{1,4}/g,"")),t.normalize&&"string"==typeof t.normalize&&(a=a.normalize(t.normalize).replace(/[\u0300-\u036f]/g,"")),o="dlatk"===t.mode?Array.from(a.matchAll(wt),(c=>c[0])):Array.from(a.matchAll(it),(c=>c[0])),!o||0===o.length)return e&&"function"==typeof e?e(void 0,[]):[];if(!1===t.preserveCase){const c="dlatk"===t.mode?dt:tt;o.forEach(((r,e,o)=>{c.test(r)||(o[e]=r.toLocaleLowerCase(t.locale))}))}return!0===t.tag&&(o=function(c,r){return c.map((c=>({value:c,tag:Et(c,r)})))}(o,t.mode)),e&&"function"==typeof e?e(void 0,o):o}}));
