 ...
 */function _html2unicode(r){r=r.replace("&amp;"," and ");const e=/&#\d+;/.exec(r);e&&e.length>0&&e.forEach((c=>{r=r.replace(c,String.fromCharCode(parseInt(c.substring(2),10)))}));const t=/&\w+;/.exec(r);return t&&t.length>0&&t.forEach((e=>{r=r.replace(e,(r=>void 0!==c[r]?String.fromCodePoint(...c[r].codepoints):r)(e))})),r}function _getTag(c,i){return"dlatk"===i?n.test(c)?"phone":d.test(c)?"url":h.test(c)?"url_scheme":l.test(c)?"url_authority":u.test(c)?"url_path_query":g.test(c)&&(!p.test(c)||p.test(c)&&/[a-zA-Z]{2,}/g.test(c))?"htmltag":p.test(c)?"emoticon":m.test(c)?"username":f.test(c)?"hashtag":b.test(c)?/\w/.test(c)?"word":"punct":"<UNK>":r.test(c)?"phone":t.test(c)&&(!e.test(c)||e.test(c)&&/[a-zA-Z]{2,}/g.test(c))?"htmltag":e.test(c)?"emoticon":a.test(c)?"username":o.test(c)?"hashtag":s.test(c)?/\w/.test(c)?"word":"punct":"<UNK>"}function tokenize(c,r,t){if(!c||"string"!=typeof c){if(t&&"function"==typeof t)return t("HappyNodeTokenizer: input must be a string.",[]);throw new TypeError("HappyNodeTokenizer: input must be a string.")}const a=function _validateOpts(c){const r={...q,...c},e="string"!=typeof r.locale?"en-US":r.locale;r.locale=e;const t="string"!=typeof r.mode?"stanford":r.mode.toLowerCase();if(!/dlatk|stanford/.exec(t))throw new SyntaxError('Tokenizer mode must be "stanford" or "dlatk".');r.mode=t;const a="string"!=typeof r.normalize?void 0:r.normalize.toUpperCase();if(a&&!/NFK?(C|D)/.exec(a))throw new SyntaxError(`Tokenizer "normalize" option must be undefined | 'NFC' | 'NFD' | 'NFKC' | 'NFKD', found "${r.normalize??""}".`);return r.normalize=a,r.preserveCase=void 0===r.preserveCase||"boolean"!=typeof r.preserveCase||r.preserveCase,r.tag=void 0!==r.tag&&"boolean"==typeof r.tag&&r.tag,r}(r);let o,s=_html2unicode(c);if("dlatk"===a.mode&&(s=function _removeHex(c){return c.replace(/\\x[0-9a-z]{1,4}/g,"")}(s)),a.normalize&&"string"==typeof a.normalize&&(s=s.normalize(a.normalize).replace(/[\u0300-\u036f]/g,"")),o="dlatk"===a.mode?Array.from(s.matchAll(w),(c=>c[0])):Array.from(s.matchAll(i),(c=>c[0])),!o||0===o.length)return t&&"function"==typeof t?t(void 0,[]):[];if(!1===a.preserveCase){const c="dlatk"===a.mode?p:e;o.forEach(((r,e,t)=>{c.test(r)||(t[e]=r.toLocaleLowerCase(a.locale))}))}return!0===a.tag&&(o=function _createTokenObject(c,r){return c.map((c=>({value:c,tag:_getTag(c,r)})))}(o,a.mode)),t&&"function"==typeof t?t(void 0,o):o}export{w as dlatkTokenizerPattern,i as stanfordTokenizerPattern,tokenize};
