...
(async () => {
  'use strict';
  const happynodetokenizer = require('../index');
  ...
  const asyncTests = async (opts) => {
    console.log('Running async tests...\n');
    const results = [];
    for (const str of testStrings) {
      const res = await happynodetokenizer.tokenize(str, opts);
      results.push(res);
    }
    const errors = compareResults(results, opts);
    if (errors > 0) {
      console.error(`Async tests finished with ${errors} error(s).\n`);
      return;
    }
    console.log('Async tests finished with 0 errors.\n');
  };
  ...
  const syncTests = (opts) => {
    ...
    const results = [];
    for (const str of testStrings) {
      const res = happynodetokenizer.tokenizeSync(str, opts);
      results.push(res);
    }
    const errors = compareResults(results, opts);
    if (errors > 0) {
      console.error(`Sync tests finished with ${errors} error(s).\n`);
      return;
    }
  ...
  const main = async () => {
    ...
    await asyncTests({mode: 'dlatk'});
    syncTests({mode: 'dlatk'});
